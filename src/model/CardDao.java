...
public class CardDao implements CardSelectable{
	...
	/**
	 * cards에서 parameter로 전달받은 num에 해당하는 숫자만큼의 카드를 random하게 선택하여 Collection타입으로 리턴해준다.
	 * @param num 원하는 카드의 개수 만큼 parameter로 전하여 사용할 수 있다.
	...
	/**
	 * 현재 cards에 새로운 path, answer에 해당하는 card를 추가한다.
	 * 이때 card의 num은 현재의 카드모음중 가장큰 num값에 +1 한 값으로 저장하여 card를 추가한다.
	 * @param path 추가할 카드그림의 이미지 경로에 대한 정보이다.
	 * @param answer 추가할 카드의 정답에 대한 정보이다.
	 ...
	 * @param card 추가하고 싶은 Card타입의 변수이다.
	 ...
	 * @param num 정답비교를 원하는 카드의 해당하는 카드번호를 parameter로 전하여 사용할 수 있다.
	 * @param answer num에 해당하는 카드의 정답과 비교할 answer이다.
	 ...
	 * @param num 카드의 해당하는 카드번호를 parameter로 전하여 사용할 수 있다.
	 ...
	 * @param num 카드번호를 parameter로 전하여 해당 카드의 index를 찾아준다.
	 ...
	 * @param num path와 answer를 수정할 카드의 번호이다.
	 * @param path 수정할 카드 그림의 경로이다.
	 * @param answer 수정할 카드의 정답이다.
	 * @return parameter로 받은 num 과 같은 카드번호에 해당하는 카드의 그림과 정답을 수정한다.이 때 성공 했을 경우 true, 실패 했을 경우 false를 리턴한다.
	 */
	 ...
	 * @param num answer를 수정할 카드의 번호이다.
	 * @param answer 수정할 카드의 정답이다.
	 * @return parameter로 받은 num 과 같은 카드번호에 해당하는 카드의 정답을 수정한다.이 때 성공 했을 경우 true, 실패 했을 경우 false를 리턴한다.
	 */
	 ...
	 * @param num 삭제할 카드의 번호이다.
	 * @return parameter로 받은 num 과 같은 카드번호에 해당하는 카드를 삭제한다.이 때 성공 했을 경우 true, 실패 했을 경우 false를 리턴한다.
	 */
...
//	}
	 ...
	 * @return CardIO클래스를 통해 읽어오기를 성공했을 때 true, 실패했을 떄 false를 리턴한다. 이때 cards는 인스턴스화 되어있어야 한다.
	 */
	 ...
	 * @return CardIO클래스를 통해 쓰기 작업을 성공했을 때 true, 실패했을 떄 false를 리턴한다. 이때 cards는 인스턴스화 되어있어야 한다.
	 */
