...
public class RankDao {
	 ...
	 * 
	 * @param name
	 *            랭킹에 추가될 이름을 전달하는 변수이다.
	 * @param score
	 *            랭킹에 추가될 점수를 전달하는 변수이다.
	 * @return 삽입에 성공하면 true, 아니면 false를 반환한다.
	 */
	public String[] insert(String name, int score) {
		if (name == null) {
			System.out.println("RankDao에서 name=null이므로 insert 실패");
			return null;
		}
		int ranking = this.getRanking(score);
		int index = this.searchInsertIndex(score);
		if (ranking == -1 || index == -1) {
			System.out.println("RankDao에서 score때문에 insert 실패");
			return null;
		}
		...
		for (int i = index + 1; i < ranks.size(); i++) {
			this.update(i, ranks.get(i).getRanking() + 1);
		}
		saveRank();
		String[] rankInfo = new String[] { "" + index, "" + ranking };
		return rankInfo;// 추가할 rank정보(index와 ranking)를 String[] 타입으로 만들어 RankAddService로 넘겨줌
	}
	...
	public int search(int score) {
		...
		System.out.println("RankDao에서 search()에서 -1 리턴");
		return -1;
	}
	...
	private int searchInsertIndex(int score) {
		if (ranks == null || score < 0) {
			System.out.println("RankDao에서 searchInsertIndex()에서 -1 리턴");
			return -1;
		}
		if (ranks.size() == 0) {
			System.out.println("RankDao에서 searchInsertIndex()에서 0 리턴");
			return 0;
		}
		for (int i = 0; i < ranks.size(); i++) {
			if (score == ranks.get(i).getScore()) {
			...
			} else if (score > ranks.get(i).getScore()) {
				return i;
			}
	...
	public boolean update(int index, int ranking) {
		if (index >= ranks.size() || index < 0) {
			System.out.println("RankDao에서 update(index, ranking)에서 false 리턴");
			return false;
		}
	 ...
	 *            사용자가 수정할 ArrayList의 index이다.
	 * @param name
	 *            사용자가 수정할 Rank의 name이다.
	 * @return ArrayList의 크기보다 index가 크거나 점수가 음수이면 false를 반환, 아니면 true를 반환한다.
	 */
	public boolean update(int index, String name) {
		if (index >= ranks.size() || index < 0) {
			System.out.println("RankDao에서 update(index, name)에서 false 리턴");
			return false;
		}
		if (name == null) {
			System.out.println("RankDao에서 update(index, name)에서 name이 null");
			return false;
		}
	...
	public boolean delete(int index) {
		if (ranks == null) {
			System.out.println("RankDao에서 delete에서 ranks가 null");
			return false;
		}
	...
	public boolean clear() {
		if (ranks == null) {
			System.out.println("RankClearService의 clear() 메소드의 rankDao가 null");
			return false;
		}
		ArrayList<String> clear = new ArrayList<String>();
		clear.add("0"); // 남아있는 랭크정보 개수(저장 정책)
		if (rankio.saveRank(clear)) { // 파일에서 clear 성공 후 Ranks에서 clear하도록 설계함
			ranks.clear();
			return true;
		}
		return false;
	...
	public ArrayList<Rank> getRanks() {
		if (ranks == null) {
			System.out.println("RankDao에서 getRanks() 메소드의 ranks가 null");
			return null;
		}
	...
	public int getRanking(int score) {
		if (score < 0) {
			System.out.println("RankDao에서 getRanking() 메소드의 score가 0보다 작음");
			return -1;
		}
		if (ranks == null || ranks.size() == 0) {
			System.out.println("RankDao에서 getRanking() 메소드의 ranks때문에 1 리턴");
			return 1;
		}
		for (int i = 0; i < ranks.size(); i++) {
			if (score >= ranks.get(i).getScore()) {
	...
	public boolean saveRank() {
		if (ranks == null) {
			System.out.println("RankDao에서 saveRank() 메소드의 ranks가 null");
			return false;
		}
